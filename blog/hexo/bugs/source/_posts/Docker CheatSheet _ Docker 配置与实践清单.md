---
title: Docker CheatSheet | Docker 配置与实践
date: 2018-09-19 08:36:03
tags: [docker]
categories: [docker]
---

> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 https://juejin.im/post/5ba11f635188255c8473a134?utm_source=gold_browser_extension

> 📖 节选自 [Awesome CheatSheet/Docker CheatSheet](https://link.juejin.im?target=https%3A%2F%2Fparg.co%2Fo9d)，对来自[官方文档](https://link.juejin.im?target=https%3A%2F%2Fdocs.docker.com%2F)及 [Docker Links](https://link.juejin.im?target=https%3A%2F%2Fparg.co%2Fo90) 中链接内容的归档整理，包含了日常工作中常用的 Docker 概念与命令，如果对于 Linux 常用操作尚不熟悉的可以参考 [Linux Commands CheatSheet](https://link.juejin.im?target=https%3A%2F%2Fparg.co%2FoiT)。
...
![](https://user-gold-cdn.xitu.io/2018/9/18/165ed623f104dbaf?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
![](https://user-gold-cdn.xitu.io/2018/9/18/165ed623ffd95804?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

Docker 综合运用了 Cgroup, Linux Namespace，Secomp capability, Selinux 等机制，在 [Docker Internals CheatSheet](https://link.juejin.im?target=.%2FDocker-Internals-CheatSheet) 中我们会有详细的讨论，或者前往 [Backend Boilerplate/docker](https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2Fwxyyxc1992%2FBackend-Boilerplate%2Ftree%2Fmaster%2Fdocker) 浏览常见服务 / 应用的 Docker 配置案例。

![](https://user-gold-cdn.xitu.io/2018/9/18/165ed623f11d39b7?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

# 安装与配置

## Docker CE

这里我们使用[科大的 Docker CE 源](https://link.juejin.im?target=https%3A%2F%2Fmirrors.ustc.edu.cn%2Fhelp%2Fdocker-ce.html)进行安装：
...
鉴于 gcr 域名的不可用，我们需要利用 [ss-privoxy](https://link.juejin.im?target=https%3A%2F%2Fhub.docker.com%2Fr%2Fbluebu%2Fshadowsocks-privoxy%2F) 等工具搭建 Docker 源代理，也可以参考[这里](https://link.juejin.im?target=https%3A%2F%2Fwww.jianshu.com%2Fp%2F13f4b23824d8)手动配置客户端：
...
Docker 推荐的是单个容器执行单个任务的关注点分离策略，容器的主进程会负责管理所有的子进程。如果我们未在自定义初始化脚本中考虑太多子进程生命周期管理的操作，那么可以使用 `--init` 参数来允许 Docker 自动注入 init 进程作为主进程，其会在容器关闭时候自动处理所有的派生的子进程，并且相对于完整的 sysvinit 或者 systemd 更为轻量级。如果我们希望在单个容器中运行多个进程，则可以使用 supervisord 或者[自定义脚本](https://link.juejin.im?target=https%3A%2F%2Fdocs.docker.com%2Fconfig%2Fcontainers%2Fmulti-service_container%2F):
...
![](https://user-gold-cdn.xitu.io/2018/9/18/165ed623f1044836?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
*   --dns-search=DOMAIN 设定容器的搜索域，当设定搜索域为 .example.com 时，在搜索一个名为 host 的 主机时，DNS 不仅搜索 host，还会搜索 [host.example.com](https://link.juejin.im?target=http%3A%2F%2Fhost.example.com) 。 注意：如果没有上述最后 2 个选项， Docker 会默认用主机上的 /etc/resolv.conf 来配置容器。
...
![](https://user-gold-cdn.xitu.io/2018/9/18/165ed623f124b15d?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
*   [InfraS-Lab/Focker](https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2Fwxyyxc1992%2FInfraS-Lab): 仿 Docker 轻量级实现

*   [深入浅出分布式基础架构](https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2Fwxyyxc1992%2FDistributed-Infrastructure-Series)。

*   [Kubernetes CheatSheet](https://link.juejin.im?target=.%2FKubernetes-CheatSheet.md): Kubernetes 基础概念，配置使用与实践技巧
